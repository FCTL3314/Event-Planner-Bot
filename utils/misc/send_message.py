...
async def send_unable_execute_stop_command_message(message: aiogram.types.Message):
    await message.answer(text="❕Вы не создаёте событие, что бы отменить его создание.")


async def send_preview_of_event_post(message: aiogram.types.Message, event_picture_id, event_name, event_description):
    if event_picture_id:
